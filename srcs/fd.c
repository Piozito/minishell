...
int handle_redir_input(char *file) 
{
...
int handle_redir_output(char *file) 
{
...
int handle_redir_output_append(char *file) 
{
...
void apply_redirections(t_env *cmds) 
{
    ...
    while (cmds->arg[i]) 
	{
        if ((ft_strcmp(cmds->arg[i], "<") == 0 ||
            ...
            if (ft_strcmp(cmds->arg[i], "<") == 0) 
			{
                if (handle_redir_input(cmds->arg[i+1]) == -1)
                    perror("input error");
            }
            else if (ft_strcmp(cmds->arg[i], ">") == 0) 
			{
                if (handle_redir_output(cmds->arg[i+1]) == -1)
                    perror("output error");
            }
            else if (ft_strcmp(cmds->arg[i], ">>") == 0) 
			{
                if (handle_redir_output_append(cmds->arg[i+1]) == -1)
                    perror("append error");
            }
            else if (ft_strcmp(cmds->arg[i], "<<") == 0) 
			{
                if (handle_redir_input_heredoc(cmds->arg[i+1]) == -1)
